...
`data-once` pauses an element's arrival until it is first seen. It needs `enter.js`: an off-screen element's animation is paused before its first frame, and `enter.js` plays it through the first time an `IntersectionObserver` sees the element cross into view — once released this way, the element is never touched again, so scrolling away and back does not replay it. Without the script `data-once` does nothing and the element simply arrives on load, since the module only pauses and resumes an animation `enter.css` is already running; it never starts, stops or picks a different one. With `data-stagger` the same pause and later play reach each child's own animation, since the module reads `getAnimations({ subtree: true })` rather than the element's own.

Where a scroll timeline exists, `data-view` wins over `data-once` on the same element: `enter.js` leaves a scroll-linked animation entirely alone, since it is already CSS's own doing, and `data-once` only ever covers the fallback, where `view()` is unsupported or the reader has asked for less motion.
