...
Each panel is a `details` with a `summary`, so the opening, the keyboard, and the announced state come from the browser rather than from Yeti. The base layer dresses a bare `details` already — a strong summary, a chevron that turns, a panel that grows open — so the component adds only what makes a column of them one object: the border around the set, the line between rows, the summary's own surface, and the padding.
