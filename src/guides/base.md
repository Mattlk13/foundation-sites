...
A native control's own colours — a checkbox or radio's fill, a range thumb, a progress bar, and the text caret — follow the palette too, through `accent-color` and `caret-color` set once on the root.
