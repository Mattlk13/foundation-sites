...
| `data-once` | boolean |  |  | Arrive once, the first time the element comes into view. Needs enter.js; without it the element arrives on load. |
...
- Purely visual, and safe by default: the element is only ever hidden by the animation's own backwards fill, so an element whose animation never runs is visible rather than stranded, and nothing here changes reading order or focus order. Under reduced motion the duration and the stagger step both collapse, so everything is simply present at once; the scroll-driven form is switched off entirely for the same reader, because a scroll timeline paces itself and would ignore a collapsed duration. data-once pauses an element's arrival until it is first seen; under reduced motion an unseen element still waits at the start, and arrives the moment it is seen, only instantly once it does, since it is the same collapsed animation being paused and resumed. Where a scroll timeline exists, data-view wins over data-once on the same element: the module leaves a scroll-linked animation alone entirely, and data-once only ever covers the fallback.
