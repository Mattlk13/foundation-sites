...
Three of them take a side, and the word differs because the geometry does. `data-side` is `start` or `end`: which edge of its trigger a `dropdown` panel lines up with, as it is which child a `sidebar` treats as the sidebar. `data-placement` is one of four: which side of its trigger a `tooltip` sits on. `data-edge` is `top`, `bottom`, or `both`: which edge a `seam` cuts. A two-value side, a four-value placement, a cut edge.
